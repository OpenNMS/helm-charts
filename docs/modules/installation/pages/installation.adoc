...
+
On GitHub, see https://github.com/OpenNMS/helm-charts/blob/main/opennms/values.yaml[values.yaml] for example configuration parameters you can specify.
...
If you wish to customize your deployment, such as if you want to use one chart to deploy multiple instances into a cloud datacenter, you can call an additional yaml and/or include `--set` parameters to override values in the yaml file.
...
== Updating OpenNMS service settings

If you make changes to the Helm chart definition, such as modifying `values.yaml`, updating one of the init scripts, or changing a container image version, you can redeploy the container by doing an upgrade of the helm chart.
