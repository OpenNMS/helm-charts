...
:!sectids:
:description: Learn how to install OpenNMS Helm Charts on Kubernetes so you can run OpenNMS Horizon locally in a Kubernetes cluster: Debian/Ubuntu, RHEL.
...
For information on installing Horizon on Red Hat OpenShift, see xref:installation:openshift.adoc[].
